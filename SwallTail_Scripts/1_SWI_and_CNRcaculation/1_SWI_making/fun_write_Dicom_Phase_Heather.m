function fun_write_Dicom_Phase_Heather(Data_dir,matrix_size,TE,unwrapped_phase_img)
...
Unwrapped_Phase = permute(unwrapped_phase_img,[2,1,3,4]); %change row/column order due to differences in representations in DICOM and Matlab
Unwrapped_Phase1 = int16(Unwrapped_Phase*1000);
...
sliceNum = size(Unwrapped_Phase1,3);
echoNum=size(Unwrapped_Phase1,4); %%% Added, Heather
...
mkdir('Unwrapped_Phase_Results'); %% Heather, added
saveDir_Final=strcat(saveDir,'\Unwrapped_Phase_Results');%% Heather, added
...
%     dicomwrite(QSM_Phase1(:,:,sliceIdx),[saveDir '/' num2str(sliceIdx) '.dcm'], info); %% Heahter, comment out
        dicomwrite(Unwrapped_Phase1(:,:,sliceIdx,echoIdx),[saveDir_Final_Echo '/' num2str(sliceIdx) '-' num2str(echoIdx) '.dcm'], info); 
    end
    imIdx = imIdx + 1;
 end
end
