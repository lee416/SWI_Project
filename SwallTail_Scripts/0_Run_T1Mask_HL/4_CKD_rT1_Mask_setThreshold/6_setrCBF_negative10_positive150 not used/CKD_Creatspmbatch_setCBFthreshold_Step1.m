...
for ss=1:size(Subjects,2)%% the last two s0204, s0206 are CKD controls
    ...
    matlabbatch{1,ss}.spm.util.imcalc.input={filename_data_1};
    %matlabbatch{1,ss}.spm.util.imcalc.output=strcat('CBF_Mask_s',int2str(Subjects(ss)),'.img');
    matlabbatch{1,ss}.spm.util.imcalc.output=strcat('CBF150_Mask_ceman_simpleoutlier_s',int2str(Subjects(ss)),'.img');
    matlabbatch{1,ss}.spm.util.imcalc.outdir={pathname_2};
    matlabbatch{1,ss}.spm.util.imcalc.expression=strcat('i1>=-10&i1<=150');
end
...
%     %--- Load Autism_rT1Mask_batch_Sample_Step1.mat to see the format and index to be edited in the following matlabbatch ---%
%     matlabbatch{1,ss}.spm.util.imcalc.input={filename_data_1};
%     matlabbatch{1,ss}.spm.util.imcalc.output=strcat('CBF_Mask_R',int2str(Subjects(ss)),'.img');
%     matlabbatch{1,ss}.spm.util.imcalc.outdir={pathname_2};
%     matlabbatch{1,ss}.spm.util.imcalc.expression=strcat('i1>=-10&i1<=150');
% end

save(['CKD_Create_CBFThreshold_batch_Step1.mat'],'matlabbatch');
