...
for ss=1:size(Subjects,2)%% the last part of controls are autism control
  ...
  if Subjects(ss)<10
     pathname_2=strcat(pathname_1,'\','Sub_00',int2str(Subjects(ss)),'\Scan',int2str(Scan_num(ssq)),'\SWI');
   ...
   elseif Subjects(ss)<=99&Subjects(ss)>=10
     pathname_2=strcat(pathname_1,'\','Sub_0',int2str(Subjects(ss)),'\Scan',int2str(Scan_num(ssq)),'\SWI');
     ...
     pathname_2=strcat(pathname_1,'\','Sub_',int2str(Subjects(ss)),'\Scan',int2str(Scan_num(ssq)),'\SWI');
...
save(['Create_rT1Mask_batch_Step2_TRY.mat'],'matlabbatch');
