...
function [num_pairs]=batch_outlier_clean_Heather(pathname,Sub_num)
...
%          files=spm_get('files',PAR.subs(sb).ses(b).condirs{c}, ['cbf_' num2str(PAR.subtractiontype) '_s*img']);
         ...
         %%s.\n',PAR.subjects{sb});continue;end % comment out, Heather
         v=spm_vol(files);
         %dat=spm_read_vols(v); %% comment out, Heather
         dat_0=spm_read_vols(v); %% added, Heather
         %siz=size(dat); %% comment out, Heather
         siz=size(dat_0); %% added, Heather
         ...
         for qq=1:siz(4)
             dat_1(:,qq)=reshape(dat_0(:,:,:,qq),siz(1)*siz(2)*siz(3),size(files,1)); %% added, Heather
             dat(:,1)=dat_1(:,qq);
...
% only use the head motion information
         ...
         cmean_1(:,:,:,qq)=cmean(:,:,:);%% added, Heather         
         dd=squeeze(mean(mean(mean(cmean_1)))); %% added, Heather
         dd_1=find(dd~=0); %% added, Heather
         cmean_corrected_4D=cmean_1(:,:,:,dd_1); %% added, Heather
         cmean_corrected=mean(cmean_corrected_4D,4); %% added, Heather
...
%          cmean(find(nanmsk>0))=nan;
         ...
         %vo=spm_write_vol(vo,cmean); %% comment out, Heather
         vo=spm_write_vol(vo,cmean_corrected); %% added, Heather
         num_pairs=size(dd_1,1); %% added, Heather
         end %% added, Heather
