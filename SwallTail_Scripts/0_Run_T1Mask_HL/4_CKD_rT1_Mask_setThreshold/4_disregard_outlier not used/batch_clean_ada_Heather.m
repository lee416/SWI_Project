...
function [image_pair_left]=batch_clean_ada_Heather(pathname,Sub_num)
...
%             movefil = spm_select('FPList', PAR.asldirses{sb,ses} , ['^rp_' '\w*.*_split.txt']); % comment out, Heather           
...
%% end, comment out, Heather
            ...
            movefil=strcat(pathname,'rp_pcasl_rest000.txt');% added, Heather
            movefil=movefil(1,:);
            moves = spm_load(movefil);
%             moves=moves(1:nsca,:);
            moves(:,10:12)=moves(:,10:12)*180/pi;
            datsum=0;
            %PP=spm_select('FPList', PAR.asldirses{sb,ses}, ['^cbf_0_sCompcorr_r\w*.*M0CSF\.nii$']); % Heather, comment out
            PP =strcat(pathname,'cbf_0_rpcasl_rest000_4D.img'); %% added, Heather
...
%             loc=(crelmov>th1)+(cavgmov>th2);
            ...
            %maskimg = spm_select('FPList', PAR.asldirses{sb,ses},'^mask_bet.nii$'); % comment out, Heather
            maskimg=strcat(pathname,'R',Sub_num,'_rT1_mask.img');
...
%             roiidx= 1:length(orgmcbf);
            ...
            image_pair_left=sum(aidx);%% added, Heather
            ndat=dat(:,aidx);
...
%             imgbuf(imgbuf>100)=100;
            vo=v(1);
            %vo.fname=fullfile(spm_str_manip(vo.fname,'H'),['cmeanCBF_ada.nii']); % comment out, Heather
            vo.fname=fullfile(spm_str_manip(vo.fname,'H'),['cmeanCBF_ada.img']); % added, Heather
