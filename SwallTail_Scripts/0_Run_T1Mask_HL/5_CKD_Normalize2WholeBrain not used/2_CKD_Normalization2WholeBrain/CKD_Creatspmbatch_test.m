Subjects=[7 142 146	154	177 179	188	194	195	198	201	202	218	225	242	243	244	258	264	320	329	330 101 103 112 118 153 204 216 217 227 331 204 206]; %% the last two, s204, s206 are CKD control%
pathname_1='/Volumes/Untitled/Autism_Data/Subjects';

%{'/Volumes/Untitled/Autism_Data/Subjects/R0007/pcasl_rest/R7_rT1_mask.img,1';}
%{'/Volumes/Untitled/Autism_Data/Subjects/R0007/pcasl_rest/meanCBF_0_rpcasl_rest078.img,1';}

%%% Step 1
%------------- this is for Autism and controls -------------------%
for ss=1:size(Subjects,2)-2%% the last two s0204, s0206 are CKD controls
    if Subjects(ss)<10
     pathname_pcasl=strcat(pathname_1,'/','R','000',int2str(Subjects(ss)),'/pcasl_rest/');
     pathname_anat=strcat(pathname_1,'/','R','000',int2str(Subjects(ss)),'/anat_anlz/');
     filename_data_1=strcat(pathname_pcasl,'Masked_Thresholded_rpcasl_CBF_R',int2str(Subjects(ss)),'.img');
     filename_data_2=strcat(pathname_anat,'ranat_anlz_seg_sn.mat');
     elseif Subjects(ss)<=99&Subjects(ss)>=10
     pathname_pcasl=strcat(pathname_1,'/','R','00',int2str(Subjects(ss)),'/pcasl_rest/');
     pathname_anat=strcat(pathname_1,'/','R','00',int2str(Subjects(ss)),'/anat_anlz/');
     filename_data_1=strcat(pathname_pcasl,'Masked_Thresholded_rpcasl_CBF_R',int2str(Subjects(ss)),'.img');
     filename_data_2=strcat(pathname_anat,'ranat_anlz_seg_sn.mat');
    else 
     pathname_pcasl=strcat(pathname_1,'/','R','0',int2str(Subjects(ss)),'/pcasl_rest/');
     pathname_anat=strcat(pathname_1,'/','R','0',int2str(Subjects(ss)),'/anat_anlz/');
     filename_data_1=strcat(pathname_pcasl,'Masked_Thresholded_rpcasl_CBF_R',int2str(Subjects(ss)),'.img');
     filename_data_2=strcat(pathname_anat,'ranat_anlz_seg_sn.mat');
    end
    
    %--- Load Autism_rT1Mask_batch_Sample_Step1.mat to see the format and index to be edited in the following matlabbatch ---%
    matlabbatch{1,ss}.spm.spatial.normalise.write.subj.matname={filename_data_2};
    matlabbatch{1,ss}.spm.spatial.normalise.write.subj.resample={filename_data_1};
end

pathname_1_CKD='/Volumes/Untitled/CKD_data/Subjects';

% %--------- this is for CKD controls -------------------------%
for ss=(size(Subjects,2)-1):size(Subjects,2)%% the last two s0204, s0206 are CKD controls
    if Subjects(ss)<10
     pathname_pcasl=strcat(pathname_1_CKD,'/','s','00',int2str(Subjects(ss)),'/pcasl_rest/');
     pathname_anat=strcat(pathname_1_CKD,'/','s','00',int2str(Subjects(ss)),'/anat_anlz/');
     filename_data_1=strcat(pathname_pcasl,'Masked_Thresholded_rpcasl_CBF_s',int2str(Subjects(ss)),'.img');
     filename_data_2=strcat(pathname_anat,'ranat_anlz_seg_sn.mat');
     elseif Subjects(ss)<=99&Subjects(ss)>=10
     pathname_pcasl=strcat(pathname_1_CKD,'/','s','0',int2str(Subjects(ss)),'/pcasl_rest/');
     pathname_anat=strcat(pathname_1_CKD,'/','s','0',int2str(Subjects(ss)),'/anat_anlz/');
     filename_data_1=strcat(pathname_pcasl,'Masked_Thresholded_rpcasl_CBF_s',int2str(Subjects(ss)),'.img');
     filename_data_2=strcat(pathname_anat,'ranat_anlz_seg_sn.mat');
    else 
     pathname_pcasl=strcat(pathname_1_CKD,'/','s',int2str(Subjects(ss)),'/pcasl_rest/');
     pathname_anat=strcat(pathname_1_CKD,'/','s',int2str(Subjects(ss)),'/anat_anlz/');
     filename_data_1=strcat(pathname_pcasl,'Masked_Thresholded_rpcasl_CBF_s',int2str(Subjects(ss)),'.img');
     filename_data_2=strcat(pathname_anat,'ranat_anlz_seg_sn.mat');
    end
    
    %--- Load Autism_rT1Mask_batch_Sample_Step1.mat to see the format and index to be edited in the following matlabbatch ---%
    matlabbatch{1,ss}.spm.spatial.normalise.write.subj.matname={filename_data_2};
    matlabbatch{1,ss}.spm.spatial.normalise.write.subj.resample={filename_data_1};
end

save(['Autism_Create_Normalization_batch.mat'],'matlabbatch');

%% open the above-created batch and run all subjects at the same time or follow run saved batch without spmGUI
%--- run saved batch without spmGUI ---%
% spm_jobman('run',matlabbatch);


