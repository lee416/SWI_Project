...
%     filename_data=strcat('rpcasl_rest0',int2str(0),int2str((s)),'.img');
    else 
    filename_data=strcat('rpcasl_rest0',int2str((s-1)),'.img'); 
%     filename_data=strcat('rpcasl_rest0',int2str((s)),'.img'); 
    end
    spmImg.InputFullpath =strcat(pathname,filename_data);
    Filename_0(s,:)=spmImg.InputFullpath;
    %tmp_data(:,:,:,s) = spm_read_vols(spm_vol(spmImg.InputFullpath));
end
%close(h)

Filename(1:(s-2),:)=Filename_0(3:s,:);
