...
for ss=1: size(Subjects,2)
     for ssq=1:size(Scan_num,2)
               if Subjects(ss)<10
    pathname=strcat(pathname_1,'\Sub_00',int2str(Subjects(ss)),'\Scan',int2str(Scan_num(ssq)),'\SWI_Data\MatData_All');
   elseif Subjects(ss)<=99&Subjects(ss)>=10
    pathname=strcat(pathname_1,'\Sub_0',int2str(Subjects(ss)),'\Scan',int2str(Scan_num(ssq)),'\SWI_Data\MatData_All');
   else 
    pathname=strcat(pathname_1,'\Sub_',int2str(Subjects(ss)),'\Scan',int2str(Scan_num(ssq)),'\SWI_Data\MatData_All');
               end
