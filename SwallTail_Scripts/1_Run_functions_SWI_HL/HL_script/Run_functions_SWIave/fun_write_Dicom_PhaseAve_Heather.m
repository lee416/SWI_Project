function fun_write_Dicom_PhaseAve_Heather(Data_dir,matrix_size,unwrapped_phase_img_ave)
...
SWI_img_ave=unwrapped_phase_img_ave;
SWI_IMG= permute(SWI_img_ave,[2,1,3]); %change row/column order due to differences in representations in DICOM and Matlab
SWI_IMG1 = int16(SWI_IMG*1000);
...
mkdir('Phase_IMG_Ave_Results'); %% Heather, added
saveDir_Final=strcat(saveDir,'\Phase_IMG_Ave_Results');%% Heather, added
...
%    %%%% end, added, Heather
 while sliceIdx < sliceNum
    ...
    if fid>0
        ...
        %--
        info.SeriesDescription = 'Phase IMG';
...
%     dicomwrite(QSM_Phase1(:,:,sliceIdx),[saveDir '/' num2str(sliceIdx) '.dcm'], info); %% Heahter, comment out
        dicomwrite(SWI_IMG1(:,:,sliceIdx),[saveDir_Final '/' num2str(sliceIdx) '.dcm'], info); 
    end
    imIdx = imIdx + 1;
 end
% end
