...
%   low_pass_img(:,:,iss,ess)=fftshift(ifft2(fftshift(kimg(:,:,iss,ess).*(filter_k))));%在k-space下與low-pass filter相乘
...
%  end
% end
% figure; imagesc(abs(low_pass_img(:,:,display_slice,echo_display))');axis image; axis off;colormap gray
% 
...
% end
% end
% figure; imagesc((high_pass_phase1(:,:,display_slice,echo_display))');axis image; axis off;colormap gray
