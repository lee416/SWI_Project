...
pathname_1='C:\Users\TMU\Desktop\Subjects';

for ss=1: size(Subjects,2)
  for ssq=1:size(Scan_num,2)
   if Subjects(ss)<10
    pathname=strcat(pathname_1,'\','Sub_','00',int2str(Subjects(ss)),'\Scan',int2str(Scan_num(ssq)),'\SWI_Data');
   elseif Subjects(ss)<=99&Subjects(ss)>=10
    pathname=strcat(pathname_1,'\','Sub_','0',int2str(Subjects(ss)),'\Scan',int2str(Scan_num(ssq)),'\SWI_Data');
   else 
...
figure;imagesc(flipud(rot90(abs(SWI_img(:,:,26,echo_display(ss))))));axis image; axis off;colormap gray
